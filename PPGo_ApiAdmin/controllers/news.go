...
func (self *NewsController) List() {
	...
	class_id ,_ := self.GetInt("class_id")		//表示的是分类
	fmt.Println("资讯列表中class_id内容：",class_id)
	pageSize := 10
	filters := make([]interface{}, 0)
	filters = append(filters, "status", 1)
	fmt.Println("fileters:",filters)
	result, count := models.ClassGetList(1, pageSize, filters...)		//获取的是分类，下拉列表分类
	fmt.Println("result:",result)
	//这里用道的一个方法是，我需要将一个结构体对象存储道一个切片中，我的切片元素数量，可以设置为len（）个；
	classList := make([]map[string]interface{}, len(result))
	for k, v := range result {		//循环这个切片map
		fmt.Println("v:",v)
		fmt.Println("v:",&v)
		row := make(map[string]interface{})
		row["id"] = v.Id		//这里的v就是结构体对象，应该是一个内存地址才对的
